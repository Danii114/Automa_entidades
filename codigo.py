...
NITS_SQL = ", ".join([f"'{nit}'" for nit in NITS])
...
SELECT
    ...
    detalle_del_objeto_a_contratar, modalidad_de_contratacion,
    cuantia_proceso, plazo_de_ejec_del_contrato,
    departamento_entidad, municipio_entidad,
    fecha_de_cargue_en_el_secop, estado_del_proceso,
    ruta_proceso_en_secop_i
WHERE nit_de_la_entidad IN ({NITS_SQL})
AND upper(estado_del_proceso) = 'CONVOCADO'
AND fecha_de_cargue_en_el_secop >= '{fecha_inicio}T00:00:00'
AND fecha_de_cargue_en_el_secop <= '{fecha_fin}T23:59:59'
"""

df_secop1 = descargar_socrata(URL_SECOP1, query_secop1)
print(f"  ✅ {len(df_secop1)} registros")
...
SELECT
    ...
    fecha_de_inicio_del_contrato, estado_contrato,
    duraci_n_del_contrato, modalidad_de_contratacion, urlproceso
WHERE nit_entidad IN ({NITS_SQL})
AND fecha_de_inicio_del_contrato >= '{fecha_inicio}T00:00:00'
AND fecha_de_inicio_del_contrato <= '{fecha_fin}T23:59:59'
"""

df_secop2 = descargar_socrata(URL_SECOP2, query_secop2)
print(f"  ✅ {len(df_secop2)} registros")

# =========================================================
# LIMPIEZA
# =========================================================

COLS_S1 = ["nombre_entidad","nit_de_la_entidad","detalle_del_objeto_a_contratar",
           "modalidad_de_contratacion","cuantia_proceso","plazo_de_ejec_del_contrato",
           "departamento_entidad","municipio_entidad","fecha_de_cargue_en_el_secop",
           "estado_del_proceso","ruta_proceso_en_secop_i"]
COLS_S2 = ["nombre_entidad","nit_entidad","id_contrato","descripcion_del_proceso",
           "valor_del_contrato","fecha_de_inicio_del_contrato","estado_contrato",
           "duraci_n_del_contrato","modalidad_de_contratacion","urlproceso"]
...
ANCHOS = {
    "nombre_entidad":35, "nit_de_la_entidad":18, "nit_entidad":18,
    "detalle_del_objeto_a_contratar":55, "descripcion_del_proceso":55,
    "modalidad_de_contratacion":25, "cuantia_proceso":18, "valor_del_contrato":18,
    "plazo_de_ejec_del_contrato":14, "departamento_entidad":20, "municipio_entidad":20,
    "fecha_de_cargue_en_el_secop":22, "fecha_de_inicio_del_contrato":22,
    "estado_del_proceso":18, "estado_contrato":18,
    "duraci_n_del_contrato":18, "id_contrato":20,
    "ruta_proceso_en_secop_i":16, "urlproceso":16,
}
...
def build_top5_rows(df, url_col):
    ...
    for _, row in top5.iterrows():
        ...
        link_html = f"<a href='{link}' style='color:#1F4E79;font-weight:bold;text-decoration:none;'>&#128279; Ver</a>" if link else "&#8212;"
...
# Métricas con <table> en lugar de display:grid (compatible con todos los clientes de correo)
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="border-bottom:1px solid #e5e7eb;">
    <tr>
      <td width="25%" style="padding:12px 6px;text-align:center;border-right:1px solid #e5e7eb;">
        ...
        <p style="margin:2px 0 0;font-size:10px;color:#888;letter-spacing:.4px;">OPORTUNIDADES</p>
      </td>
      <td width="25%" style="padding:12px 6px;text-align:center;border-right:1px solid #e5e7eb;">
        <p style="margin:0;font-size:24px;font-weight:700;color:{text_color};">{entidades}</p>
        <p style="margin:2px 0 0;font-size:10px;color:#888;letter-spacing:.4px;">ENTIDADES</p>
      </td>
      <td width="25%" style="padding:12px 6px;text-align:center;border-right:1px solid #e5e7eb;">
        <p style="margin:0;font-size:14px;font-weight:700;color:#1a7f4b;">${v_total:,.0f}</p>
        <p style="margin:2px 0 0;font-size:10px;color:#888;letter-spacing:.4px;">VALOR TOTAL</p>
      </td>
      <td width="25%" style="padding:12px 6px;text-align:center;">
        <p style="margin:0;font-size:14px;font-weight:700;color:#1a7f4b;">{vp}</p>
        <p style="margin:2px 0 0;font-size:10px;color:#888;letter-spacing:.4px;">VALOR PROMEDIO</p>
      </td>
    </tr>
  </table>
  <div style="padding:14px 20px 16px;">
    <p style="margin:0 0 8px;font-size:13px;font-weight:700;color:#1F4E79;">&#128176;&nbsp; Top 5 por valor</p>
    <table width="100%" cellpadding="0" cellspacing="0">
      <thead>
        <tr style="background:#f8fafc;">
          <th style="padding:6px;text-align:left;font-size:10px;color:#888;border-bottom:1px solid #e5e7eb;">ENTIDAD</th>
          <th style="padding:6px;text-align:left;font-size:10px;color:#888;border-bottom:1px solid #e5e7eb;">DESCRIPCI&#211;N</th>
          <th style="padding:6px;text-align:left;font-size:10px;color:#888;border-bottom:1px solid #e5e7eb;">VALOR</th>
          <th style="padding:6px;text-align:center;font-size:10px;color:#888;border-bottom:1px solid #e5e7eb;">LINK</th>
        </tr>
      </thead>
...
s1 = seccion_html("SECOP I &#8212; Estado CONVOCADO",
                  "#1e40af", "#EFF6FF", "#bfdbfe",
                  t1, e1, vt1, vp1, rows1)
s2 = seccion_html("SECOP II &#8212; Contratos por fecha de inicio",
...
<table width="680" cellpadding="0" cellspacing="0"
  ...
  <!-- PIE -->
  <tr><td style="background:#f8fafc;padding:10px 20px;text-align:center;
